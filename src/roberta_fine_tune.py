...
def eval(model, eval_loader, device, criterion=nn.CrossEntropyLoss(), with_labels=True):
    ...
    return probs
#python roberta_fine_tune.py --batch_size 8 --fname roberta_snli --output_dir roberta_ckpts_snli/ --task_name snli
def main():
    ...
    hf_datasets = ['imdb', 'sst2', 'mnli', 'hans', 'snli', 'rte', 'yelp_polarity']
    glue = ['sst2', 'mnli', 'rte']
    ...
    if args.train_file is None:
        if args.task_name in hf_datasets:
            ...
            train_df = pd.read_table(os.path.join(os.getcwd(), 'train' + args.file_format))
