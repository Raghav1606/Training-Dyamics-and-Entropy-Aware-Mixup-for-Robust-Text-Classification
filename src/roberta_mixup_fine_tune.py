...
--mixup_use_label \
--mixup_use_entropy \
--device 2 > /projects/metis2/atharvak/Data_Cartography/out_files/disaster/modeling_roberta_mixup_use_none_category_use_label_use_entropy_disaster_flag_gelu.out &
nohup python3 -u roberta_mixup_fine_tune.py \
--task_name disaster \
--roberta_version roberta-base \
--include_none \
--mixup_type category \
--mixup_use_label \
--device 0 > /projects/metis2/atharvak/Data_Cartography/out_files/disaster/modeling_roberta_mixup_use_none_category_use_label_disaster_flag_gelu.out &
    ...
    for epoch in train_iterator:
        ...
        if epoch + 1 == MIXUP_START:
            ...
            if arguments.mixup_type == 'random':
                ...
                train_data_processed = prepare_dataset_random_mixup(train_data, info_data=info_df,
                                                                    include_none=arguments.include_none,
                                                                    use_label=arguments.mixup_use_label)
            elif arguments.mixup_type == 'category':
                print("\n\nCategory-based mixup processing...\n")
                train_data_processed = prepare_dataset_category_mixup(train_data, info_data=info_df,
                                                                      include_none=arguments.include_none,
                                                                      use_label=arguments.mixup_use_label,
                                                                      use_entropy=arguments.mixup_use_entropy)
...
def main():
    ...
    parser.add_argument('--mixup_type', type=str, default='random', help='How to sample data',
                        choices=('random', 'category'))
    ...
    parser.add_argument('--file_format', type=str, default='.tsv',
                        help='Data file format for tasks not available for download at HuggingFace Datasets')
    ...
    model = RobertaMixerForSequenceClassification.from_pretrained(args.roberta_version,
